...
- **Known gap**: these four `:domainId` routes only require *some* valid JWT (`JwtAuthGuard`) — they don't check that the caller's own account matches the target `domainId`. Any authenticated user can currently act on any other user's account. Acceptable for now (no roles/permissions concept exists yet), but needs closing before this app has more than one real user.
...
- **Authorization / roles**: needed to close the `:domainId` gap noted above.
