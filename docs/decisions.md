...
See `functionality.md` for the up-to-date implemented-vs-planned breakdown. Backend (all 5 modules) and frontend (auth + all 4 domain CRUD features, sidebar shell) are both feature-complete for v1's planned scope; dashboard/reporting, `users` self-profile UI, and the smaller items (CSV/Plaid import, roles, password reset, tests) remain.
