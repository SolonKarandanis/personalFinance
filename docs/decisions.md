...
- **Category and Account both get a Lookup store** alongside their Search/Detail pair, despite all three reading from the same `GET /categories`/`GET /accounts` endpoints: `CategoryLookupStore`/`AccountLookupStore` (shared — minimal, just the list, used by Transaction/Budget form dropdowns) vs. `CategorySearchStore`+`CategoryDetailStore`/`AccountSearchStore`+`AccountDetailStore` (the dedicated "manage" CRUD screens). Different UI jobs, so different stores, per the granularity rule's own self-check ("could a list view and an edit view import this store independently? They should import different stores"). `AccountLookupStore` was originally left off the per-domain store plan below (an oversight from before `transactions` was designed) and added once `transactions` became its first real consumer — `CategoryLookupStore` was similarly deferred until then for the same reason (no consumer yet).
- Per-domain stores, all built: `accounts` → Search+Detail+Lookup; `categories` → Search+Detail+Lookup; `transactions` → Search+Detail (Detail handles both normal `create` and `createTransfer`, since a transfer is still "a Transaction" once it exists; `TransactionSearchStore` is the first Search store with real filter state — `accountDomainIdFilter`, feeding `GET /transactions?accountDomainId=`); `budgets` → Search+Detail (no Lookup — nothing looks up budgets); `users` → `UserDetailStore` only (always "my own profile", no search needed — and the only Detail store with no sibling Search store, so it owns both its own read and write state directly rather than splitting them, naming its read signals `profileLoading`/`profileError` to avoid colliding with `withCallState()`'s write-side `loading`/`error`); `dashboard` → `DashboardStore`, the read-only aggregate/dashboard category (no further suffix — see below). `AuthStore` is the one cross-cutting exception to the granularity categories (holds the in-memory access token; the refresh token itself lives in the httpOnly cookie already built on the backend).
- **Cross-feature store/service imports use the `@app/*` path alias, not relative paths.** Hit a real bug building `transactions`: `TransactionsService` importing `AccountLookupStore`/`CategoryLookupStore` via a relative path (`../../accounts/data/store/...`) was off by one directory level (`services/` → `data/` → `transactions/` → `app/` is three `../`, not two). The wrong-but-resolvable-looking path produced a confusing cascade of `TS2571: Object is of type 'unknown'` and implicit-`any` errors at every downstream usage, not a clear "module not found" — worth remembering if that error shape shows up again. Fixed by switching to `@app/accounts/data/store/account-lookup.store` / `@app/categories/data/store/category-lookup.store`; use the alias for any future cross-feature-folder import.
...
## Dashboard: one new endpoint, not a subsystem

Reviewing what "dashboard/reporting" actually needed turned out to need far less new backend work than the original scope note implied. Net worth (sum of `Account.currentBalance`) and budgets-vs-actual (`Budget.currentPeriodSpent`/`amount`) were already fully exposed by existing endpoints — the frontend just recombines already-built `AccountLookupStore`/`BudgetSearchStore` data, no new API calls. The only genuinely new aggregations were **spending by category** and **income/expense trend over time**, neither of which existed anywhere (`TransactionsService.findAll` only returns raw rows). Both live behind a single new endpoint, `GET /dashboard/summary`, in a new `dashboard` module — a `DashboardService` running two `QueryBuilder` aggregate queries against `Transaction` (joined to `Account`, filtered by `userId`, both excluding `type = 'transfer'` per the already-established rule that transfer rows never count in expense/income totals). This is the first place this doc's original ORM choice ("use TypeORM's QueryBuilder/raw SQL for analytics queries") actually got exercised.

- **Category breakdown** is for the current calendar month, reusing `getCurrentPeriodRange('monthly', now)` from `budgets/budget-period.util.ts` rather than reinventing month-boundary math.
- **Monthly trend** covers the last 6 calendar months and is always returned zero-filled for months with no activity — computed by cross-referencing the query's grouped rows against a generated list of the last 6 `YYYY-MM` keys, not by trusting the query to produce a row for every month (it won't, for months with zero transactions).
- No new entity, no migration — purely read-only aggregation over `Transaction`.

### Frontend: hand-rolled SVG charts, no charting library

Used the `dataviz` skill's method and reference palette rather than adding a charting dependency (ngx-charts, Chart.js, D3, etc.) — consistent with this app's dependency-minimal approach everywhere else (e.g. the budget progress bar is two styled `<div>`s, not a component). The category-breakdown bar list and the income/expense trend line chart are both plain HTML/SVG, hand-built per the skill's mark specs (2px lines, 4px rounded bar ends, ≥8px end markers, hairline recessive gridlines).

**Color**: this app's own Spartan theme has no hue (the "neutral" preset's `--primary` is pure grayscale, chroma 0), so there was no existing brand color to preserve — used the dataviz skill's validated reference palette directly. Income/expense (2 categorical series, a trend over time) take slots 1 (blue) and 2 (orange); the category-magnitude bars use the sequential blue ramp instead of per-category hues (comparing magnitude across potentially many named categories — already axis-labeled — is exactly the "sequential, one hue" case from the skill's form table, not identity/categorical, and sidesteps the 8-color categorical cap entirely). Ran `scripts/validate_palette.js` for both light and dark before using either pair — both pass every check (worst-case CVD ΔE 24.7 light / 26.8 dark, well above the ≥8 target). Colors are wired as CSS custom properties on a `.viz-root` wrapper, themed via `:host-context(.dark) .viz-root` — this app's actual dark-mode mechanism is a `.dark` class on `:root` (Spartan/Tailwind default), not the skill's literal `data-theme` attribute/media-query example, so the selector was adapted to match rather than copied verbatim.

**Deliberately simplified** vs. the skill's full interaction spec: native SVG `<title>` tooltips instead of a custom crosshair/synced-tooltip layer, and no separate table-view twin. Every value is still direct-labeled (bar tips, line endpoints, axis labels), so nothing is gated behind hover — but it's a lighter interaction layer than the skill's documented default (a full hover/crosshair system is "part of the deliverable, not an upgrade"). Revisit if this dashboard grows more series or gains a date-range picker.
...
See `functionality.md` for the up-to-date implemented-vs-planned breakdown. Backend (all 6 modules, including `dashboard`) and frontend (auth, all 4 domain CRUD features, self-profile, dashboard, sidebar shell) are both feature-complete for the originally planned v1 scope. What remains is the smaller/lower-priority items: CSV/Plaid import, roles, password reset, multi-session tokens, a shared-types package, and tests (none written yet, either side).
